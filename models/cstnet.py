...
def sample_and_group(n_center, n_near, xyz, mad, adj, pt, fea):

    # idx_surfknn_all = utils.surface_knn(xyz, n_near, 10)
    idx_surfknn_all = utils.knn(xyz, n_near)
    ...
    if n_center is None:
        ...
        canter_eula = index_points(mad, fps_idx)
        center_near = index_points(adj, fps_idx)
        center_meta = index_points(pt, fps_idx)

        idx = index_points(idx_surfknn_all, fps_idx)

    g_xyz = index_points(xyz, idx)  # [B, npoint, nsample, 3]
    g_eula = index_points(mad, idx)
    g_near = index_points(adj, idx)
    g_meta = index_points(pt, idx)
...
def sample_and_group_all(xyz, mad, adj, pt, fea):
    # B: batch_size, N: point number, C: channels
    B, N, C = xyz.shape

    g_xyz = xyz.view(B, 1, N, C)
    g_eula = mad.view(B, 1, N, -1)

    g_near = adj.view(B, 1, N, -1).repeat(1, 1, 1, 2)
    g_meta = pt.view(B, 1, N, -1).repeat(1, 1, 1, 2)
...
class Embed(nn.Module):
    ...
    def forward(self, xyz, mad, adj, pt, fea=None):
        # xyz: [bs, n_point, 3]
        # mad: [bs, n_point, 3]
        ...
        if self.group_all:
            center_xyz, center_eula, center_near, center_meta, center_fea, new_fea = sample_and_group_all(xyz, mad, adj, pt, fea)
        else:
            # xyz: torch.Size([24, 1024, 3])
            center_xyz, center_eula, center_near, center_meta, center_fea, new_fea = sample_and_group(self.n_center, self.n_near, xyz, mad, adj, pt, fea)
...
class CstNet(nn.Module):
    ...
    def forward(self, xyz, mad, adj, pt):
        ...
        orig_fea = torch.cat([xyz, mad, adj, pt], dim=-1).transpose(1, 2)
        orig_fea = self.preprocess(orig_fea).transpose(1, 2)

        l1_xyz, l1_eula, l1_near, l1_meta, l1_fea = self.sa1(xyz, mad, adj, pt, orig_fea)
